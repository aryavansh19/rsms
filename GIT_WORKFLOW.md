...
How everyone pushes/pulls to keep `main` clean and avoid conflicts.

## 1. Branch model
```
main      ← protected. Always builds & runs. Only release merges. NEVER push directly.
 └─ develop   ← integration branch. Everyone's features merge here first.
     ├─ feature/TEAM5-31-qr-sale
     ├─ feature/TEAM5-60-admin-login
     └─ feature/TEAM5-...     ← ONE branch per Jira story
```
Create `develop` once (whoever set up the repo):
```bash
git checkout main && git pull
git checkout -b develop
git push -u origin develop
```
Then in GitHub → Settings → Branches: protect `main` AND `develop` (require Pull Request + 1 approval + "build passes").
...
git checkout develop          # work starts from develop, not main
```
...
# (4) STAY IN SYNC — pull develop into your branch at least once a day
git pull --rebase origin develop      # replays your commits on top of latest develop

# (5) push your branch
git push -u origin feature/TEAM5-31-qr-sale

# (6) open a Pull Request on GitHub:  your branch → develop
...
## 5. How to minimize conflicts (the important part)
...
4. **Rebase on develop daily** (`git pull --rebase origin develop`) so you're never far behind.
5. **Pull develop right before opening a PR** so the merge is clean.
...
## 6. The Xcode `.pbxproj` problem (read this!)
...
- Add a `.gitattributes` file at the repo root containing:
  ```
  *.pbxproj merge=union
  *.pbxproj -text
  ```
  `merge=union` keeps additions from BOTH sides instead of conflicting.
...
- Never commit: `xcuserdata/`, `*.xcuserstate`, `DerivedData/`, `build/` (put them in `.gitignore`).

## 7. If you get a conflict
```bash
...
git push --force-with-lease  # only your own feature branch, never develop/main
```
For a conflicted `.pbxproj`: easiest fix is often to accept both additions (it's a list), then in Xcode do **File → Packages → Reset / Clean Build Folder** and confirm the project opens.
...
- End of sprint: open a release PR `develop → main`, tag `v0.1.0`.

## 9. Golden rules (pin these)
- Never `git push` to `main` or `develop` directly — always via PR.
- Never force-push a shared branch (`main`, `develop`). Only your own feature branch.
