...
75 user stories from `team5_user_stories_only.txt`, keyed by Jira ID.

**Conventions:** Priority = MoSCoW. Story Points = Fibonacci (1,2,3,5,8). Baseline story = TEAM5-58 (1 pt); reference story = TEAM5-2 (3 pts).
...
## Sprint Capacity Plan (proportional to hours)

Uniform velocity = 840 hrs / 222 pts = **3.78 hrs per story point**. 
Each sprint's point budget is its hours divided by this rate, so points track hours directly.

| Sprint | Theme | Stories | Points | Capacity (hrs) | Points x 3.78 |
|--------|-------|---------|--------|----------------|----------------|
| 1 | Foundation + Admin/Manager setup | 22 | 55 | 210 | 208 |
| 2 | Commerce engine + transfers + events | 26 | 93 | 350 | 352 |
| 3 | After-sales + analytics + submissions | 27 | 74 | 280 | 280 |
| **Total** | | **75** | **222** | **840** | **840** |

_Week 1 (Sprint 1) = 55 pts ≈ 208 hrs, matching the 210 hr capacity. Epics assigned whole where possible; only Inter-store Transfers (intake -> S1, execution -> S2) and Analytics (manager-store view -> S1, revenue dashboard -> S3, since it needs sales data) are split by dependency._

## Sprint 1 (22 stories, 55 pts)
...
| TEAM5-60 | Admin | As an Admin, I want to log in with my credentials, so that I can access only the features and data relevant to my role. | Auth & RBAC | 3 |
...
| TEAM5-2 | Admin | As an Admin, I want to create a new store profile with name, address, and contact details, so that the store is registered in the system and visible across the platform. | Store Onboarding | 3 |
...
| TEAM5-78 | After-Sales | As an After-Sales Associate, I want to schedule customer pickups so that completed items are returned in an organized manner. | Return & Collection | 2 |
